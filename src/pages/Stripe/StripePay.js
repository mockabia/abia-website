...
const publishKey = await apiService
  .apiCall(apiUrls.STRIPE_API.STRIPE_PUBLISHKEY, "GET")
...
let succesUrl =
  window.location.origin + process.env.REACT_APP_URL + "payment-success";
//let options = {};
...
const validateForm = (fields, setError) => {
  ...
  if (
    fields.holdername == "" ||
    fields.holdername == undefined ||
    fields.holdername == null
  ) {
    validate = false;
    setError((values) => ({
      ...values,
      ["holdername"]: "Card holder name is required",
    }));
  }
  ...
  if (
    ...
    setError((values) => ({
      ...values,
      ["condition"]: "Please agree with terms and conditions",
    }));
  }
  return validate;
};
const PaymentForm = (props) => {
  ...
  const handleSubmit = (stripe, elements) => async () => {
    ...
    if (validateForm(fields, setError)) {
      ...
      let tokenData = "";
      stripe
        .createToken(cardElement, { name: fields.holdername })
        .then(function (response) {
  ...
  const paymentMethod = async (cardElement, token, tokenData) => {
    ...
    if (error) {
      ...
      await apiService
        .apiCall(paymentAPI + "/" + fields.vid, "POST", totalResponse)
        .then(function (response) {
          if (response.statuscode == 200) {
  ...
  const checkMultipleEmailAccounts = async (email) => {
    ...
    request["email"] = email;
    await apiService
      .apiCall(apiUrls.BUSINESS_API.MULTIPLE_ACCOUNTS_EMAIL, "GET", request)
      .then(function (response) {
        if (response.statuscode == 200) {
      ...
      <div>
        <FormControlLabel
          ...
          label={
            ...
            >
              I{" "}
              <span style={{ textTransform: "lowercase" }}>Agree To The </span>
              <span style={{ textTransform: "capitalize", color: "#6cc2bc" }}>
...
const StripePaymentForm = (props) => {
  ...
  let options = {
    mode: "payment",
    amount: paymentuser.payamount
      ? parseFloat(paymentuser.payamount)
      : parseFloat(0),
    country: "AUS",
    currency: "usd",
  };
